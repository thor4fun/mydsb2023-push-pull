...
```{r}
#setting tables up as db object
...
#sneak peaking what I am dealing with
members_dm %>% 
  glimpse()

#to answer the question:
members_df %>% 
  # add new column member_id to join with payment_df
  mutate(member_id = id) %>% 
  # join members_df with payments_df by matching member_id
  right_join(payments_df,by="member_id") %>% 
  # group by MP name
  group_by(name) %>% 
  # sum received money for each MP
  summarise(total_money = sum(value)) %>% 
  # arrange MP who have more money first
  arrange(desc(total_money))

# Seems that Ms May collected the most
...
payments_df %>% 
  ...
  group_by(entity) %>% 
  # sum total money that each entity donate
...
# Looks like Withers LLP is the only one that breached the 5% mark, with 1.8m donations.
```
...
members_df %>% 
  # add new column member_id to join with payment_df
  mutate(member_id = id) %>% 
  # join members_df with payments df by matching member_id
  right_join(payments_df,by="member_id") %>% 
  # select to show only entity and party_id column
  select(entity,party_id) %>% 
  # group by entity and party_id
  group_by(entity,party_id) %>% 
  # count each entity paired with party_id
  summarise(count=n()) %>% 
...
# Out of 2213 entities, 2036 entities donated to a single party. Its 2023/2213 = 0.92 of the total.
```
...
snake_case %>% 
  janitor::clean_names()
# Im trying to run the functions provided in the homework
...
write.csv(map(page_generator(pages),scrap_jobs) %>% map_dfr(bind_rows), "/Users/szkol/Desktop/all_consulting_jobs.csv", row.names=FALSE)
